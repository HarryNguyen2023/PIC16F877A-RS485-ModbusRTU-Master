# 1 "PIC16F877A_RS485_ModbusRTU_master.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "PIC16F877A_RS485_ModbusRTU_master.c" 2
# 1 "./config.h" 1
...
# 2 "PIC16F877A_RS485_ModbusRTU_master.c" 2
...
void main(void) {
    ...
    while(1)
    {
        ...
        {
            case 0:
                if(! modbusReadCoils(tx_buffer, 25, READ_COILS, 0x05, 1005, 7))
                    RS485sendString(error_msg);
                break;
            case 1:
                if(! modbusReadCoils(tx_buffer, 25, READ_DISCRETE_INPUTS, 0x05, 1001, 7))
                    RS485sendString(error_msg);
                break;
            case 2:
                if(! modbusReadRegisters(tx_buffer, 25, READ_HOLDING_REGISTERS, 0x05, 4003, 3))
                    RS485sendString(error_msg);
                break;
            case 3:
                if(! modbusReadRegisters(tx_buffer, 25, READ_INPUT_REGISTERS, 0x05, 4005, 2))
                    RS485sendString(error_msg);
                break;
            case 4:
                if(! modbusWriteCoil(tx_buffer, 25, 0x05, 1003, COIL_ON))
                    RS485sendString(error_msg);
                break;
            case 5:
                if(! modbusWriteRegister(tx_buffer, 25, 0x05, 4001, 15))
                    RS485sendString(error_msg);
                break;
            case 6:
                if(! modbusWriteCoils(tx_buffer, 25, 0x05, 1001, 13, coil_write_state))
                    RS485sendString(error_msg);
                break;
             case 7:
                state = 0;
                if(! modbusWriteRegisters(tx_buffer, 25, 0x05, 4002, 2, register_write_buffer))
                    RS485sendString(error_msg);
                break;
        }
